    ...
    public class BegetSMTPEmailSender : IEmailSender
    {
            ...
            CancellationToken token)
        {
            if (_smtpClient.IsConnected == false)
            {
                ...
                throw new InvalidOperationException("BegetSMTPEmailSender already connected");
        }
            ...
            CancellationToken token)
        {
            if (_smtpClient.IsAuthenticated == false)
            {
                ...
                throw new InvalidOperationException("BegetSMTPEmailSender already authenticated");
        }
        public async Task DisconnectAsync(bool quit)
        {
            if (_smtpClient.IsConnected == true)
            {
                ...
                throw new InvalidOperationException("BegetSMTPEmailSender already disconnected");
        }
        ...
        {
            if (_smtpClient.IsConnected == false)
                throw new InvalidOperationException("BegetSMTPEmailSender is not connected");
            if (_smtpClient.IsAuthenticated == false)
                throw new InvalidOperationException("BegetSMTPEmailSender is not authenticated");
            ...
            {
                _logger.LogInformation($"Ответ smtp сервера: сообщение не доставлено");
            }
        }
    }
}
