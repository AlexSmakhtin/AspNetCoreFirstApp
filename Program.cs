...
namespace AspNetCoreFirstApp
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            //app.MapGet("/", () => DateTime.UtcNow.ToString() + " Дата на сегодн€");
            //app.MapGet("/new_year", () => Math.Round((DateTime.Parse($"01.01.{DateTime.Now.Year + 1}") - DateTime.Now)
            //.TotalDays).ToString() + " дней осталось до Ќового года");

            //1
            app.MapGet("/customs_duty", (double price) =>
            {
                string message = "»тогова€ стоимость посылки с пошлиной: ";
                int limit = 200;
                if (price <= limit)
                    return message + price.ToString() + " И";
                return message + (price += price * 0.15) + " И";
            });
